...
class Systray:
	...
	def load_icons(self):
		self.debug(text="def load_icons()")
		# called from: 
		"""
		...
		try:
			...
			for name,value in sorted(self.d0wns_DNS.iteritems()):
				try:
					...
					self.debug(text="def make_context_menu_servertab_d0wns_dnsmenu: dnsmenu.append(dnssubm) '%s' failed "%(countrycode))
					...
					if not countrycodefrombefore == countrycode:
						...
						try:
							...
							try:
								...
								self.debug(text="def make_systray_server_menu: countrycode = '%s' failed" % (countrycode))
								self.destroy_systray_menu()
						except:
							self.destroy_systray_menu()
	...
	def base64_icons(self, icon):
		self.debug(text="def base64_icons(%s)"%(icon))
		...
		elif icon == "disconnect1":
			return disconnect1_b64
		
		elif icon == "disconnect2":
			return disconnect2_b64
		
		elif icon == "disconnect3":
			return disconnect3_b64
		
		elif icon == "disconnect1_menu":
			return disconnect1_menu_b64
		
		elif icon == "disconnect3_menu":
			return disconnect3_menu_b64
		
		elif icon == "testing":
			return testing_b64
		
		elif icon == "connected_classic":
